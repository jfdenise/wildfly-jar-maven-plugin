...
public class DevWatchContextTestCase {
    ...
    public void testUpdateWebAppsFiles() throws Exception {
        ...
        try {
            ...
            BootableAppEventHandler handler = checkEvent(ctx, projCtx, false, false, false, false, false);
            ...
            handler = checkEvent(ctx, projCtx, false, false, false, false, false);
...
//            System.out.println("DELETED, WAITING 5 secs");
//            Thread.sleep(5000);
            handler = checkEvent(ctx, projCtx, false, false, false, false, false);
    ...
    public void testUpdateJavaFilesWeb() throws Exception {
        ...
        try {
            ...
            Files.write(newFile, "Hello".getBytes());
            BootableAppEventHandler handler = checkEvent(ctx, projCtx, false, true, false, false, true);
            ...
            handler = checkEvent(ctx, projCtx, false, true, false, false, true);
            Assert.assertTrue(handler.redeploy);
            Assert.assertTrue(handler.copied.isEmpty());
            ...
            Files.createFile(existingFile);
            Files.write(existingFile, "Hello4".getBytes());
            handler = checkEvent(ctx, projCtx, true, true, false, false, true);
            Assert.assertTrue(handler.redeploy);
            Assert.assertEquals(1, handler.deleted.size());
    ...
    public void testUpdateJavaFiles() throws Exception {
        ...
        try {
            ...
            Path newFile = Files.createFile(project.javaDir.resolve("Foo2.java"));
            Files.write(newFile, "Hello".getBytes());
            BootableAppEventHandler handler = checkEvent(ctx, projCtx, false, true, false, true, false);
            Assert.assertTrue(handler.redeploy);
            Assert.assertTrue(handler.copied.isEmpty());
            Assert.assertTrue(handler.seenUpdated.contains(newFile));
            projCtx.reset();
            ...
            handler = checkEvent(ctx, projCtx, false, true, false, true, false);
            Assert.assertTrue(handler.redeploy);
            Assert.assertTrue(handler.copied.isEmpty());
            Assert.assertTrue(handler.seenUpdated.contains(existingFile));
            projCtx.reset();
            ...
            Files.createFile(existingFile);
            Files.write(existingFile, "Hello4".getBytes());
            handler = checkEvent(ctx, projCtx, true, true, false, true, false);
            Assert.assertTrue(handler.redeploy);
            Assert.assertEquals(1, handler.deleted.size());
            Assert.assertEquals(existingFile, handler.deleted.get(existingFile));
            Assert.assertTrue(handler.copied.isEmpty());
    ...
    public void testPomFile() throws Exception {
        ...
        try {
            ...
            BootableAppEventHandler handler = checkEvent(ctx, projCtx, false, false, true, false, false);
            ...
            Files.write(project.pomFile, expected.getBytes());
            handler = checkEvent(ctx, projCtx, false, false, true, false, false);
...
//            Thread.sleep(5000);
    ...
    private BootableAppEventHandler checkEvent(DevWatchContext ctx, TestProjectContext projCtx, boolean cleanup, boolean compile, boolean checkPluginConfig, boolean packageJar, boolean packageWar) throws Exception {
